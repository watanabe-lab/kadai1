...
class LoadBarancerForLevel1 < Controller
  ...
  def start
    # fdb(key, value) = (macaddr, port)
    # server_list(key, value) = (ipaddr, macaddr)
    @fdb = {}   
    @server_list = {}
  ...
  def handle_arp_request dpid, message
    ...
    dest_ip = message.arp_tpa.to_s
    puts "ARP Request from " + source_ip + " to " + dest_ip
  ...
  def handle_arp_reply dpid, message
    source_ip = message.arp_spa.to_s
    dest_ip = message.arp_tpa.to_s
    puts "ARP Reply from " + source_ip + " to " + dest_ip
